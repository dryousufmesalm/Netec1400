...
function Get-MoneyMachineWizardAccounts {
    ...
    return @(
        Import-Csv -LiteralPath $ConfigPath | ForEach-Object {
            ...
            $vpsId = if($_.PSObject.Properties['VpsId']) { [string]$_.VpsId } else { '' }
...
function Invoke-MoneyMachineWizardDesktopOperation {
    ...
    try {
        ...
        $rawResult = @(& $operationScript -Operation $Operation -RequestPath $requestPath -RuntimeRoot $canonicalRuntimeRoot -ConfigPath $ConfigPath -TerminalDataRoot $TerminalDataRoot -SkipTaskRegistration:$SkipTaskRegistration -AsLibrary)
...
function Export-MoneyMachineWizardSupportReport {
    ...
    $discovery = Invoke-MoneyMachineWizardDesktopOperation -Operation Discover -Payload ([pscustomobject]@{}) -ConfigPath $ConfigPath -RuntimeRoot $RuntimeRoot -TerminalDataRoot $TerminalDataRoot
...
function Get-MoneyMachineWizardCommandResult {
    ...
    switch($Command) {
        ...
        'discoverMt4Accounts' {
            Assert-MoneyMachineWizardPayloadFields -Payload $Payload -Allowed @()
            return Invoke-MoneyMachineWizardDesktopOperation -Operation Discover -Payload $Payload -ConfigPath $ConfigPath -RuntimeRoot $RuntimeRoot -TerminalDataRoot $TerminalDataRoot
        }
        'browseForCsv' {
            Assert-MoneyMachineWizardPayloadFields -Payload $Payload -Allowed @()
            $selected = Show-MoneyMachineWizardCsvPicker
            return Invoke-MoneyMachineWizardDesktopOperation -Operation Discover -Payload ([pscustomobject]@{ manualCsv = @($selected) }) -ConfigPath $ConfigPath -RuntimeRoot $RuntimeRoot -TerminalDataRoot $TerminalDataRoot
        }
        'getOneDriveRoots' {
            Assert-MoneyMachineWizardPayloadFields -Payload $Payload -Allowed @()
                ...
                if($path.StartsWith('/api/',[StringComparison]::OrdinalIgnoreCase)) {
                    ...
                    if($path -ceq '/api/command') {
                        ...
                        $result = Invoke-MoneyMachineSetup -Request ([pscustomobject]@{
                            ...
                            DestinationFolder = if($payload.PSObject.Properties['destinationFolder']) { [string]$payload.destinationFolder } else { '' }
