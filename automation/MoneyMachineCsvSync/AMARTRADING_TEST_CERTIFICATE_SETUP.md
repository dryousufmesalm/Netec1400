...
3. Confirm the Windows administrator prompt to install the test certificates for this VPS.
4. Run `Start-AmarTradingBrowser.cmd`.

The installer adds the included test root only to the machine Trusted Root store and its publisher certificate only to the machine Trusted Publishers store. It does not disable Smart App Control, Microsoft Defender, AppLocker, WDAC, or the PowerShell execution policy. It does not install an EXE.
