...
- `Sync/AmmarTrading.Sync.exe` — self-contained Windows sync application.
...
Double-click `Sync\AmmarTrading.Sync.exe`. On first launch, complete the system check, select the Ready MT4 account, choose the signed-in OneDrive root, validate the selection, and apply setup. The app stores logs, configuration, and WebView2 data under `%LOCALAPPDATA%\AmmarTrading\Sync`.
