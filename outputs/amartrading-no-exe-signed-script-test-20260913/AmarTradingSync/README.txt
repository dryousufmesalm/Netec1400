...
2. Run Install-AmarTradingTestCertificate.cmd once and confirm its prompt.
3. Start OneDrive and sign in with the approved uploader account.
4. Double-click Start-AmarTradingBrowser.cmd.

The certificate is installed only for the current Windows user. It lets the signed test scripts run when the VPS uses PowerShell RemoteSigned. It does not disable Smart App Control, Defender, AppLocker, WDAC, or PowerShell execution policy.
