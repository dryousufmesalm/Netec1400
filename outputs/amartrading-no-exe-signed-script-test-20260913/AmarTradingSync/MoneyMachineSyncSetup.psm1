...
function Resolve-AmmarTradingDestinationFolder {
    ...
    $rootPrefix = $resolvedRoot.TrimEnd([IO.Path]::DirectorySeparatorChar,[IO.Path]::AltDirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar
    if(-not $folder.StartsWith($rootPrefix, [StringComparison]::OrdinalIgnoreCase)) { throw 'The destination folder must be inside the selected OneDrive root.' }
    if((Get-Item -LiteralPath $resolvedRoot -Force).Attributes -band [IO.FileAttributes]::ReparsePoint) { throw 'The selected OneDrive root is not a safe local folder.' }
...
function Invoke-AmmarTradingBatchSetup {
    ...
    $identity = Get-AmmarTradingVpsIdentity -RuntimeRoot $RuntimeRoot -VpsName $vpsName
    ...
    $fullConfigPath = [IO.Path]::GetFullPath([Environment]::ExpandEnvironmentVariables($ConfigPath))
    if(Test-Path -LiteralPath $fullConfigPath -PathType Leaf) {
        $foreign = @(Import-Csv -LiteralPath $fullConfigPath -ErrorAction Stop | Where-Object { $_.PSObject.Properties['VpsId'] -and -not [string]::IsNullOrWhiteSpace([string]$_.VpsId) -and [string]$_.VpsId -cne [string]$identity.VpsId }).Count
    ...
    try {
        ...
        foreach($account in $normalizedAccounts) {
            $accountResults.Add([pscustomobject][ordered]@{
                ...
                Destination = Get-AmmarTradingDestinationPath -OneDriveRoot $account.OneDriveRoot -AccountNumber $account.ExpectedMT4Login -VpsId $identity.VpsId -DestinationFolder $account.DestinationFolder
