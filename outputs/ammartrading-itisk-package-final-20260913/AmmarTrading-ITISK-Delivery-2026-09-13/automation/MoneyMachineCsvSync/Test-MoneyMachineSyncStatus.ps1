...
function Get-AmmarTradingHeartbeatStatus {
    ...
    try {
        ...
        $destination = Get-AmmarTradingDestinationPath -OneDriveRoot $OneDriveRoot -AccountNumber $AccountNumber -VpsId $VpsId
...
function Get-AmmarTradingConfiguredSyncStatus {
    ...
    foreach($configuration in @(Import-Csv -LiteralPath $ConfigPath -ErrorAction Stop)) {
        ...
        $destination = Get-AmmarTradingDestinationPath -OneDriveRoot $configuredRoot -AccountNumber $accountNumber -VpsId $vpsId
        $heartbeat = Get-AmmarTradingHeartbeatStatus -OneDriveRoot $configuredRoot -AccountNumber $accountNumber -VpsId $vpsId -FreshnessHours $FreshnessHours
