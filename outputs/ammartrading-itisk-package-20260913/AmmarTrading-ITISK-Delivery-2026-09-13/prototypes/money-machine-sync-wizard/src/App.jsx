...
function OneDriveScreen({ state, busy, onBack, onContinue, onRetry, dispatch }) {
  ...
  return (
    <WizardLayout screen={screens.ONEDRIVE}>
      <PageHeading eyebrow="Step 3 of 5" title="Choose the OneDrive folder" copy="Reports are published into an AmarTrading account folder inside the selected local OneDrive root." />
          ...
          <div className="content-card mapping-panel">
            ...
            <div className="mapping-list">
              {selectedAccounts.map((account) => (
                <article key={discoveryId(account)}><FileCsv weight="duotone" /><span><b>Account {accountNumber(account)}</b><small>OneDrive / AmarTrading / Account_{accountNumber(account)} / Baskets.csv</small></span><CheckCircle weight="fill" /></article>
              ))}
            </div>
...
export function App() {
  ...
  async function runAutomaticFirstSetup(systemStatus) {
    ...
    try {
      ...
      const payload = buildAutomaticSetupPayload({ vpsName, accounts: eligible, oneDriveRoot });
  ...
  return (
    <div className="app-shell" dir="ltr">
      ...
      {state.screen === screens.ONEDRIVE && <OneDriveScreen state={state} busy={busy.roots} onBack={() => dispatch({ type: "SCREEN_CHANGED", screen: screens.ACCOUNTS })} onRetry={() => runAutomaticFirstSetup(state.systemStatus)} onContinue={validateSelection} dispatch={dispatch} />}
