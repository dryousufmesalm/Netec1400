...
public sealed class NativeDialogService : INativeDialogService
{
    ...
    private static string VerifyConfiguredAccountDirectory(string destination, string accountNumber)
    {
        ...
        var productDirectory = vpsDirectory is null ? null : Path.GetDirectoryName(vpsDirectory);
        var isLegacyLayout = vpsDirectory is not null && Path.GetFileName(vpsDirectory).Equals("AmarTrading", StringComparison.Ordinal);
        var isVpsLayout = vpsDirectory is not null && productDirectory is not null &&
            Path.GetFileName(vpsDirectory).StartsWith("VPS_", StringComparison.Ordinal) &&
            Path.GetFileName(productDirectory).Equals("AmarTrading", StringComparison.Ordinal);
